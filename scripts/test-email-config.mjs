...
ok(
  ...
  Boolean(process.env.ADMIN_ALLOWED_EMAILS?.trim()),
...
console.log(`  ${siteUrl}/api/admin/auth/callback`);
console.log(`  ${siteUrl}/api/email/oauth/callback`);
console.log("  http://localhost:3000/api/admin/auth/callback");
console.log("  http://localhost:3000/api/email/oauth/callback");

console.log("\n--- Manual test steps ---");
console.log("  1. npm run dev");
console.log("  2. Open http://localhost:3000/admin/login");
console.log("  3. Sign in with Google (allowed email) or admin password");
